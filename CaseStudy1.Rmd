...
fit = knn(trainAle[,c(4,5)],testAle[,c(4,5)],trainAle$Style, k=3)
...
fit = knn(trainIPA[,c(4,5)],testIPA[,c(4,5)],trainIPA$Style, k=3)
...
ggplot() + 
  ...
  geom_polygon(data = boundary, aes(x,y, color=predicted, fill=predicted), alpha = 0.5)+
  geom_point(aes(ABV, IBU, color=Style), data=test) + ggtitle("KNN Cluster Plot") + ggtitle("KNN Cluster Plot for IPA") 
```
